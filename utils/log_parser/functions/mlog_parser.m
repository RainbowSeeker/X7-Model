...
for n = 1:LogHeader.num_param_group
    ...
    LogHeader.param_group_list(n).name = LogHeader.param_group_list(n).name(isdis_str(LogHeader.param_group_list(n).name));
    ...
    for k = 1:LogHeader.param_group_list(n).num_param
        ...
        LogHeader.param_group_list(n).param(k).name = LogHeader.param_group_list(n).param(k).name(isdis_str(LogHeader.param_group_list(n).param(k).name));
    end
end
...
for n = 1:LogHeader.num_bus
    ...
    BusName = BusName(isdis_str(BusName));
...
for n = 1:LogHeader.num_bus
    ...
    for k = 1:LogHeader.bus(n).num_elem
        ...
        ElemName = ElemName(isdis_str(ElemName));
...
function [out] = isdis_str(string)
    if isstring(string) || ischar(string)
        out = logical((string > 32) .* (string < 127));
    else
        out = nan;
    end
end